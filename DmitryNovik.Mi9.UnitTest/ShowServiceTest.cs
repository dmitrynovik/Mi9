...
namespace DmitryNovik.Mi9.UnitTest
{
    [TestFixture]
    public class ShowServiceTest
    {
        [Test]
        public void When_Empty_String_Submitted_Error_Is_Returned()
        {
            var response = ExecuteTest("");
            Assert.IsFalse(string.IsNullOrWhiteSpace(response.error));
        }

        [Test]
        public void When_Null_Submitted_Error_Is_Returned()
        {
            var response = ExecuteTest("");
            Assert.IsFalse(string.IsNullOrWhiteSpace(response.error));
        }
        ...
        [Test]
        public void When_Empty_Json_Submitted_Error_Is_Returned()
        {
            var response = ExecuteTest("{}");
            Assert.IsFalse(string.IsNullOrWhiteSpace(response.error));
        }
        ...
        [Test]
        public void When_Invalid_Json_Submitted_Error_Is_Returned()
        {
            var response = ExecuteTest("{\"payload\":}");
            Assert.IsFalse(string.IsNullOrWhiteSpace(response.error));
        }

        [Test]
        public void When_Invalid_Json_Schema_Submitted_Error_Is_Returned()
        {
            var response = ExecuteTest("{\"payload1\": []}");
            Assert.IsFalse(string.IsNullOrWhiteSpace(response.error));
        }

        [Test]
        public void When_Correct_Schema_NoPayload_Submitted_No_Error_Is_Returned()
        {
            var response = ExecuteTest("{\"payload\": []}");
            Assert.IsTrue(string.IsNullOrWhiteSpace(response.error));
            Assert.AreEqual(0, response.response.Count());
        }

        [Test]
        public void When_Valid_Request_Is_Submitted_Correct_Number_Of_Shows_Is_Returned()
        {
            var sRequest = @"{
    ""payload"": [
        {
    ...
    ""totalRecords"": 75
            ...
            var response = ExecuteTest(sRequest);
            Assert.IsNull(response.error);
            Assert.AreEqual(7, response.response.Count());
        }

        private ShowResponse ExecuteTest(string sRequest)
        {
            var service = new EpisodeDrmFilterService();
            using (var requestStream = new MemoryStream(Encoding.UTF8.GetBytes(sRequest)))
            {
                return service.Filter(requestStream);
            }
        }
    }
}
